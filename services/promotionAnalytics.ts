...
export function computePromoEffectiveness(
  ...
  // CRITICAL FIX: Calculate actual days elapsed in the window to subtract baseline correctly
  const startDateObj = new Date(windows.event.start);
  const limitDateObj = new Date(limitDate);
  const diffTime = Math.abs(limitDateObj.getTime() - startDateObj.getTime());
  const calendarDaysElapsed = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
  ...
  // Use calendarDaysElapsed instead of eventLogs.length
  const upliftUnits = actualUnits - (baselineDailyUnits * calendarDaysElapsed);
  const upliftRevenue = actualRevenue - ((baselineDailyUnits * calendarDaysElapsed) * baselinePrice);
  ...
  const expectedProfitBaseline = baselineDailyUnits * calendarDaysElapsed * baselineProfitPerUnit;
