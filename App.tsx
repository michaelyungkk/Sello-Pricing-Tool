...
import { Product, PricingRules, PriceLog, PromotionEvent, UserProfile as UserProfileType, ChannelData, LogisticsRule, ShipmentLog, StrategyConfig, VelocityLookback, RefundLog, ShipmentDetail, HistoryPayload, PriceChangeRecord, AnalysisResult, SearchChip, SearchConfig } from './types';
...
import { LayoutDashboard, Settings, Bell, Upload, FileBarChart, DollarSign, BookOpen, Tag, Wifi, WifiOff, Database, CheckCircle, ArrowRight, Package, Download, Calculator, RotateCcw, List, UploadCloud, ChevronDown, Ship, Link as LinkIcon, Loader2, Wrench, Search, X, History } from 'lucide-react';
...
// ... existing handleSearch implementation
    ...
    const QuickUploadMenu = () => { const [isOpen, setIsOpen] = useState(false); const menuRef = useRef<HTMLDivElement>(null); useEffect(() => { const handleClickOutside = (event: MouseEvent) => { if (menuRef.current && !menuRef.current.contains(event.target as Node)) { setIsOpen(false); } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []); const actions = [ { label: 'Import Inventory', icon: Database, action: () => setIsUploadModalOpen(true), color: 'text-indigo-600' }, { label: 'Import Sales', icon: FileBarChart, action: () => setIsSalesImportModalOpen(true), color: 'text-blue-600' }, { label: 'Import Refunds', icon: RotateCcw, action: () => setIsReturnsModalOpen(true), color: 'text-red-600' }, { label: 'CA Report', icon: Upload, action: () => setIsCAUploadModalOpen(true), color: 'text-purple-600' }, { label: 'SKU Mapping', icon: LinkIcon, action: () => setIsMappingModalOpen(true), color: 'text-amber-600' }, { label: 'Shipment Import', icon: Ship, action: () => setIsShipmentModalOpen(true), color: 'text-teal-600' }, { label: 'Cost Update', icon: DollarSign, action: () => setIsCostUploadModalOpen(true), color: 'text-green-600' }, ]; return ( <div className="relative z-50" ref={menuRef}> <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-md transition-all font-medium" style={{ backgroundColor: userProfile.themeColor }}> <UploadCloud className="w-4 h-4" /> <span className="hidden md:inline">Upload Data</span> <ChevronDown className={`w-4 h-4 transition-transform ${isOpen ? 'rotate-180' : ''}`} /> </button> {isOpen && ( <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right"> <div className="p-2 grid gap-1"> {actions.map((item) => ( <button key={item.label} onClick={() => { item.action(); setIsOpen(false); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors text-left w-full group" > <div className={`p-1.5 rounded-md bg-gray-50 group-hover:bg-white border border-gray-100 group-hover:shadow-sm transition-all ${item.color}`}> <item.icon className="w-4 h-4" /> </div> <span className="font-medium">{item.label}</span> </button> ))} </div> </div> )} </div> ); };
    ...
    // --- COST IMPORT ---
    const handleCostImport = (data: { sku: string; cost: number; floorPrice?: number; ceilingPrice?: number }[]) => {
        setProducts(prev => prev.map(p => {
            ...
            if (update) {
                return {
                    ...
                    floorPrice: update.floorPrice,
                    ceilingPrice: update.ceilingPrice
                };
            }
            return p;
        }));
            ...
            <div className="min-h-screen flex font-sans text-gray-900 transition-colors duration-500 relative bg-transparent">
                ...
                <main className="flex-1 md:ml-64 p-8 min-w-0 relative z-10">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar relative p-4 md:p-8">
                        ...
                        {currentView === 'costs' && (<CostManagementPage products={products} onUpdateCosts={(updates) => {/*...*/}} onOpenUpload={() => setIsCostUploadModalOpen(true)} logisticsRules={logisticsRules} themeColor={userProfile.themeColor} headerStyle={headerStyle} />)}
                ...
                {isCostUploadModalOpen && <CostUploadModal onClose={() => setIsCostUploadModalOpen(false)} onConfirm={handleCostImport} />}
