...
export const PerformanceTrendTab: React.FC<PerformanceTrendTabProps> = ({
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-custom-glass rounded-xl shadow-lg border border-custom-glass overflow-hidden">
                ...
                <div className="sello-table-scroll">
                    <table className="sello-table">
                        ...
                        <tbody>
                            {portfolioAverages && (
                                <tr className="bg-theme-10/50 border-b border-indigo-100 shadow-inner">
                                    <td className="c"><Activity className="w-4 h-4 text-theme mx-auto" /></td>
                                    ...
                                    <td className="c"><span className="text-[10px] font-bold text-theme uppercase italic">System Baseline</span></td>
                                </tr>
                            )}
                            {rankedPlatforms.map((row) => {
                                ...
                                return (
                                    <tr key={row.platform} className={`cursor-pointer ${hoveredPlatform === row.platform ? 'bg-theme-10/40 ring-1 ring-inset ring-indigo-200' : ''}`} onMouseEnter={() => setHoveredPlatform(row.platform)} onMouseLeave={() => setHoveredPlatform(null)}>
            ...
            <div className="bg-custom-glass rounded-xl shadow-lg border border-custom-glass overflow-hidden">
                <div className="p-4 border-b border-custom-glass bg-gray-50/50 flex justify-between items-center"><div className="flex items-center gap-2"><Settings className="w-4 h-4 text-theme" /><h4 className="font-bold text-gray-800 text-sm">Chart Configuration</h4></div><button onClick={() => setIsGroupCreatorOpen(!isGroupCreatorOpen)} className="text-xs font-medium px-3 py-1.5 rounded-lg border bg-white text-gray-600 border-gray-200 hover:border-gray-300 transition-all flex items-center gap-1"><Plus className="w-3 h-3" /> Create Group</button></div>
                <div className="p-6">
                    {isGroupCreatorOpen && (<div className="bg-theme-10/50 p-4 rounded-lg border border-indigo-100 mb-6"><div className="flex gap-4 mb-3"><div className="flex-1"><label className="text-[10px] font-medium text-theme uppercase block mb-1">Group Name</label><input type="text" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} placeholder="e.g. Amazon Combined" className="w-full text-sm border border-theme-20 rounded-md px-3 py-1.5 bg-white" /></div></div><div className="mb-4"><label className="text-[10px] font-medium text-theme uppercase block mb-1">Select Platforms</label><div className="flex flex-wrap gap-2">{uniquePlatforms.map((p: string) => <button key={p} onClick={() => toggleNewGroupPlatform(p)} className={`px-2 py-1 text-xs rounded border transition-all ${newGroupPlatforms.includes(p) ? 'bg-theme text-white border-indigo-700' : 'bg-white text-gray-600 border-gray-200 hover:border-theme-20'}`}>{p}</button>)}</div></div><div className="flex justify-end gap-2"><button onClick={() => setIsGroupCreatorOpen(false)} className="text-xs text-gray-500 px-3 py-1.5">Cancel</button><button onClick={handleCreateGroup} disabled={!newGroupName || newGroupPlatforms.length < 2} className="text-xs bg-theme text-white font-medium px-4 py-1.5 rounded-md disabled:opacity-50">Save Group</button></div></div>)}
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-bold transition-all shadow-sm ${isSmoothed ? 'bg-theme-10 text-theme border-theme-20' : 'bg-white text-gray-500 border-gray-200 hover:border-theme-20'}`}
                                >
                                    <Sparkles className={`w-3.5 h-3.5 ${isSmoothed ? 'text-theme' : 'text-gray-400'}`} />
                                    {isSmoothed ? '7-Day Smooth Active' : 'Raw Data (Choppy)'}
                                </button>
                                {platformGroups.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">{platformGroups.map((g: any) => (<div key={g.id} className="flex items-center gap-2 px-3 py-2 bg-theme-10 text-theme text-xs font-medium rounded-lg border border-indigo-100 shadow-sm"><Layers className="w-3.5 h-3.5" />{g.name}<button onClick={() => deleteGroup(g.id)} className="ml-1 hover:text-red-500"><X className="w-3 h-3" /></button></div>))}</div>
                                ) : null}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
                        <div className="flex items-center gap-4">
                            <h3 className="font-bold text-gray-800 flex items-center gap-2 text-lg"><TrendingUp className="w-5 h-5 text-theme" />Performance Trend</h3>
                            <div className="flex bg-gray-100 p-1 rounded-lg">
                                <button onClick={() => setChartViewMode('OVERLAY')} className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all flex items-center gap-1.5 ${chartViewMode === 'OVERLAY' ? 'bg-white shadow text-theme' : 'text-gray-500 hover:text-gray-700'}`}><Maximize2 className="w-3 h-3" /> Overlay</button>
                                <button onClick={() => setChartViewMode('TRELLIS')} className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all flex items-center gap-1.5 ${chartViewMode === 'TRELLIS' ? 'bg-white shadow text-theme' : 'text-gray-500 hover:text-gray-700'}`}><LayoutGrid className="w-3 h-3" /> Trellis</button>
                            </div>
                        </div>
                        <div className="flex bg-gray-100 p-1 rounded-lg">{(['NET_PROFIT', 'MARGIN_PCT', 'AVG_ORDER_VALUE', 'UNITS_SOLD'] as const).map(m => (<button key={m} onClick={() => setTrendMetric(m)} className={`px-4 py-2 text-[10px] font-medium uppercase tracking-wider rounded-md transition-all ${trendMetric === m ? 'bg-white shadow text-theme' : 'text-gray-500 hover:text-gray-700'}`}>{metricLabels[m]}</button>))}</div>
                    </div>

                    {chartViewMode === 'OVERLAY' ? (
                        <div className="h-[450px] w-full relative group/chart select-none">
                            {zoomState.isZoomed && (<button onClick={handleResetZoom} className="absolute top-4 right-12 z-20 px-3 py-1.5 bg-white/90 backdrop-blur-sm border border-indigo-100 text-theme rounded-lg shadow-lg hover:bg-theme-10 transition-all flex items-center gap-1.5 text-xs font-medium"><RotateCcw className="w-3.5 h-3.5" />Reset View</button>)}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {selectedChartPlatforms.map((platform) => {
                                ...
                                return (
                                    <div key={platform} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 h-[240px] flex flex-col group hover:shadow-md hover:border-theme-20 transition-all">
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            <div className="text-[10px] font-black text-theme bg-theme-10 px-1.5 py-0.5 rounded border border-indigo-100">
                                                {(() => {
            ...
            <div className="bg-custom-glass rounded-xl shadow-lg border border-custom-glass overflow-hidden mt-6">
                <div className="p-4 border-b border-custom-glass bg-gray-50/50 flex justify-between items-center"><div className="flex items-center gap-2"><BarChartIcon className="w-4 h-4 text-theme" /><h3 className="font-bold text-gray-800 text-sm uppercase tracking-wide">{metricLabels[trendMetric]} Comparison by Platform</h3></div></div>
