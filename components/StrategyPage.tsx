...
const StrategyPage: React.FC<StrategyPageProps> = ({ products, pricingRules, currentConfig, onSaveConfig, themeColor, headerStyle, priceHistoryMap, promotions, priceChangeHistory = [], costChangeHistory = [], onUpdatePriceChangeRecord, onUpdateCostChangeRecord, onManualPriceChange, onManualCostChange, velocityLookback, thresholds }) => {
    ...
    const [isAuditPanelVisible, setIsAuditPanelVisible] = useState(false); // New state for audit panel
    ...
    const getCalculationWindow = (setting: string, cStart?: string, cEnd?: string) => {
        ...
        if (setting === 'Custom' && cStart && cEnd) {
            ...
            startKey = '1970-01-01'; // Epoch-like start for filtering
            endKey = yesterdayKey;
            days = 30; // Placeholder, will be calculated dynamically
        } else {
    ...
    const calculateMetrics = (product: Product, setting: string, isLocal: boolean) => {
        ...
        const history = skuLogs.filter((h: any) => {
            ...
            if (!logKey) return false; // Skip invalid dates
                    ...
                    <div className="bg-custom-glass p-4 rounded-xl border border-custom-glass shadow-sm flex flex-col xl:flex-row items-center justify-between gap-4 relative z-20 backdrop-blur-custom">
                        ...
                        <div className="flex flex-wrap items-center gap-3 justify-end w-full xl:w-auto">
                             <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold border transition-all shadow-sm text-sm ${isAuditPanelVisible ? 'bg-amber-500 text-white border-amber-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                                title="Toggle Reconciliation Panel"
                            >
                                <Activity className="w-4 h-4" />
                                Audit: {isAuditPanelVisible ? 'On' : 'Off'}
                            </button>
                            <button
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs">
                                ...
                                <div className="space-y-2 bg-white/50 p-3 rounded-lg border border-gray-200">
                                    ...
                                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-600">
                                        <dt># Logs Used:</dt><dd className="font-mono font-medium text-gray-900">{auditStats.local.logs}</dd>
                                        <dt>Total Distinct Days:</dt><dd className="font-mono font-medium text-gray-900">{auditStats.local.distinctDaysCount}</dd>
                                        <dt>Avg. Days/Product:</dt><dd className="font-mono font-medium text-gray-900">{auditStats.local.avgDistinctDaysPerProduct.toFixed(1)}</dd>
                                ...
                                <div className="space-y-2 bg-white/50 p-3 rounded-lg border border-gray-200">
                                    <h4 className="font-bold text-purple-700">Strategy Window ({velocityLookback}D)</h4>
                                     <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-600">
                                        <dt># Logs Used:</dt><dd className="font-mono font-medium text-gray-900">{auditStats.global.logs}</dd>
                                        <dt>Total Distinct Days:</dt><dd className="font-mono font-medium text-gray-900">{auditStats.global.distinctDaysCount}</dd>
                                        <dt className="opacity-0">Placeholder</dt><dd className="opacity-0"></dd>
