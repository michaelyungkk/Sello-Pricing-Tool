...
import { formatPct, formatMoney, formatSmartMoney } from '../../../utils/format';
...
export const ReturnsAndRefundsTab: React.FC<ReturnsAndRefundsTabProps> = ({
    ...
    const currentTableData = useMemo(() => {
        ...
        if ((searchTags.length > 0 || searchQuery) && viewMode === 'product') {
            const matchesTerm = (term: string, row: any) => {
                const t = term.toLowerCase();
                const product = productLookup.get(row.sku);
                return row.sku?.toLowerCase().includes(t) || product?.name?.toLowerCase().includes(t);
            };
            data = data.filter(row =>
                searchTags.length > 0
                    ? searchTags.some(tag => matchesTerm(tag, row))
                    : matchesTerm(searchQuery, row)
            );
        }

        const getValue = (row: any, key: string) => row[key];
        return sortRows(data, sortConfig, getValue);
    }, [byReason, byProduct, byPartner, viewMode, sortConfig, searchTags, searchQuery, productLookup]);
    ...
    const sortedTriageRows = useMemo(() => {
        ...
        let rows = triageOverview.skuRows;
        ...
        return sortRows(rows, triageSortConfig, getValue);
    }, [triageOverview.skuRows, triageSortConfig, searchTags, searchQuery, productLookup]);
        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    ...
                    <MetricCard
                        title="Loss Value"
                        value={formatSmartMoney(triageOverview.kpis.totalRefundValue)}
                        icon={DollarSign}
                        color="red"
