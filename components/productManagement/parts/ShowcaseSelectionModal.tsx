...
import { X, FileText, CheckSquare, Square, AlertTriangle, Clock, Stamp } from 'lucide-react';
...
export const ShowcaseSelectionModal: React.FC<ShowcaseSelectionModalProps> = ({
    ...
    // Products with no landedAt at all (need stamping)
    const unstamped = useMemo(() =>
        [...suggested, ...notReady].filter(p => !p.landedAt),
    [suggested, notReady]);
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                {/* ── Stamp landedAt for products missing it ── */}
                {unstamped.length > 0 && onStampLandedAt && (
                    <div className="mx-6 mb-3 p-3 rounded-xl border border-amber-200 bg-amber-50 flex items-center gap-3">
                        <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" />
                        <div className="flex-1 min-w-0">
                            <div className="text-xs font-bold text-amber-800">{unstamped.length} product{unstamped.length > 1 ? 's' : ''} missing a landed date <span className="font-normal text-amber-600">(showing estimated from last update)</span></div>
                            <div className="text-[10px] text-amber-600 mt-0.5">Stamp them now so future reports use the correct date.</div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                            <input
                                type="date"
                                value={stampDate}
                                onChange={e => { setStampDate(e.target.value); setStampConfirmed(false); }}
                                className="text-xs border border-amber-300 rounded-lg px-2 py-1 bg-white"
                            />
                            {!stampConfirmed ? (
                                <button
                                    onClick={() => { onStampLandedAt(unstamped.map(p => p.sku), stampDate); setStampConfirmed(true); }}
                                    className="px-3 py-1 text-xs font-bold text-white rounded-lg whitespace-nowrap"
                                    style={{ background: themeColor }}
                                >
                                    Stamp {unstamped.length} SKUs
                                </button>
                            ) : (
