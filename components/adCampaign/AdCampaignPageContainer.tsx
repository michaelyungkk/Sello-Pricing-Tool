        ...
        <div className="space-y-1 mt-1">
            ...
            <div className="flex gap-1.5">
                <button onClick={() => { onChange(draft); setEditing(false); }}
                    className="px-3 py-1 text-[10px] font-bold bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Save</button>
                    ...
                    {REMOVE_REASONS.map(r => (
                        <button key={r} onClick={() => setReason(r)}
                            className={`text-[10px] px-2.5 py-1 rounded-lg border transition-all font-medium
                                ${reason === r ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-200 text-gray-600 hover:border-indigo-300 hover:text-indigo-600'}`}>
                            {r}
                        </button>
                    ))}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl p-6 w-96 border border-gray-100 space-y-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => {
                        ...
                        className="flex-1 py-2 text-xs font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-40 flex items-center justify-center gap-1.5">
                                    ...
                                    <td className="r">
                                        <div className="flex items-center justify-end gap-1.5">
                                            <div className="w-10 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${c.score * 100}%` }} />
                                            </div>
        ...
        <div className="bg-custom-glass rounded-xl border border-custom-glass p-4 space-y-3">
            ...
            <div
                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all
                    ${dragging ? 'border-indigo-400 bg-indigo-50/30' : 'border-gray-200 hover:border-indigo-300'}`}
                ...
                {parsing && <p className="text-xs text-indigo-500 mt-2 font-medium">Parsing files…</p>}
            ...
            <div className="flex gap-2 pt-1">
                ...
                <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold text-white rounded-lg disabled:opacity-40 transition-all
                        ${mode === 'update' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
...
const AdCampaignPageContainerInner: React.FC<AdCampaignPageProps> = ({
...
const AdCampaignPageContainer = React.memo(AdCampaignPageContainerInner);
