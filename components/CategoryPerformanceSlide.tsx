                ...
                <div className={`flex flex-col bg-custom-glass backdrop-blur-custom rounded-xl border border-custom-glass shadow-sm overflow-hidden transition-all duration-300 ${selectedCell ? 'w-2/3' : 'w-full'}`}>
                    <div className="p-3 border-b border-white/10 flex justify-between items-center bg-white/10">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {viewMode === 'TREEMAP' && <div className="relative"><Globe className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-400" /><select value={treemapPlatform} onChange={(e) => setTreemapPlatform(e.target.value)} className="pl-6 pr-6 py-1 text-[10px] font-bold border border-white/20 rounded-md bg-white/10 focus:outline-none"><option value="All">Global</option>{platforms.map(p => <option key={p} value={p}>{p}</option>)}</select></div>}
