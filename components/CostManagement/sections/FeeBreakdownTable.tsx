...
export const FeeBreakdownTable: React.FC<FeeBreakdownTableProps> = ({
    ...
    const CombinedCell = ({ value, percent, isCurrency = true, highlight = false }: { value: number, percent?: number, isCurrency?: boolean, highlight?: boolean }) => {
        ...
        const top = showPercentPrimary && percent !== undefined ? `${(percent ?? 0).toFixed(2)}%` : (isCurrency ? formatSmartMoney(displayVal) : displayVal.toFixed(2));
        const bottom = showPercentPrimary && percent !== undefined ? (isCurrency ? formatSmartMoney(displayVal) : displayVal.toFixed(2)) : (percent !== undefined ? `${(percent ?? 0).toFixed(2)}%` : null);
    ...
    return (
        <div className="bg-custom-glass rounded-xl shadow-lg border border-custom-glass overflow-hidden backdrop-blur-custom">
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto relative">
                <table className="tbl tbl-compact w-full text-left text-sm whitespace-nowrap">
                    <thead>
                        <tr>
                        ...
                        {paginatedProducts.map(product => {
                            ...
                            return (
                                <tr key={product.id} className="group">
                                    <td className="px-3 py-1.5 sticky left-0 bg-white/50 backdrop-blur-sm group-hover:bg-white z-10 border-r border-gray-100">
                                    ...
                                    <td className="px-3 py-1.5 text-right">
                                        {product.caPrice ? (
                                            <span className="font-bold text-purple-600 font-mono">{formatSmartMoney((includeVat ? product.caPrice : product.caPrice / VAT_RATE))}</span>
                                        ) : '-'}
                                    </td>
                                    <td className="px-3 py-1.5 text-right">
                                    ...
                                    <td className="px-3 py-1.5 text-right sticky right-0 bg-white/90 backdrop-blur-md group-hover:bg-white z-20 border-l border-gray-100 min-w-[100px]">
