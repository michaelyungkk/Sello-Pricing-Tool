...
import { Product, CostChangeRecord } from '../types';
import { X, Save, DollarSign, Search, Tag } from 'lucide-react';

interface ManualCostChangeModalProps {
    products: Product[];
    onClose: () => void;
    onConfirm: (data: Omit<CostChangeRecord, 'id' | 'changeType' | 'percentChange'>) => void;
}

const ManualCostChangeModal: React.FC<ManualCostChangeModalProps> = ({ products, onClose, onConfirm }) => {
    ...
    useEffect(() => {
        ...
        if (product) {
            ...
            // Pre-fill old cost if it exists
            if (product.costPrice) {
                setOldCost(product.costPrice.toFixed(2));
            }
            setError('');
        } else {
    ...
    const handleSubmit = () => {
        if (!selectedProduct || !date || oldCost === '' || newCost === '') {
            setError('Please fill all fields and select a valid product.');
            return;
        }

        const oldCostNum = parseFloat(oldCost);
        const newCostNum = parseFloat(newCost);

        if (isNaN(oldCostNum) || isNaN(newCostNum)) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h2 className="text-xl font-bold text-gray-900">Lodge Manual Cost Change</h2>
...
export default ManualCostChangeModal;