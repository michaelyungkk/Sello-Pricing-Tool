                                    ...
                                    <tr key={product.id} className="hover:bg-gray-50/50 group">
                                        ...
                                        >
                                            <div className="flex flex-col items-end">
                                                <span className={`font-mono font-bold ${grossMargin < 0 ? 'text-red-600' :
                                        ...
                                        >
                                            <div className="flex flex-col items-end">
                                                <span className={`font-mono font-bold ${margin < 0 ? 'text-red-600' :
...
const CostExportModal = ({ products, onClose, themeColor }: { products: Product[], onClose: () => void, themeColor: string }) => {
    const handleDownload = () => {
        ...
        link.href = url;
        const filename = `cost_structure_export_${new Date().toISOString().slice(0, 10)}.csv`;
        link.download = filename;
        link.setAttribute('download', filename);
        document.body.appendChild(link);
        link.click();

        // Revoke after 60s delay
        setTimeout(() => {
